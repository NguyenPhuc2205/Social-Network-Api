 ...
 * controllers and other services to access translations based on the user's preferred language.
 */
@injectable()
export class I18nService implements II18nService {
   ...
   * @type {string}
   * @default 'en'
   * @description Falls back to this language when language detection fails or language is not specified
   */
   ...
   * @example
   * const t = i18nService.getTFunction(req)  // Use in controller or middleware
   * 
  ...
  public getTFunction(req: Request, language?: string): TFunction {
    ...
    return i18next.getFixedT(lng) // Creates a translation function bound to the specified language (t function)
  }

  /**
   * Translates a key into the corresponding text in the current language
   * @param {string} key - Translation key (e.g., 'common:not_found', 'user:profile.title')
   ...
   * 
   * const vietnameseMessage = i18nService.translate(
   *   'common:GREETING', 
   *   undefined, 
   *   { name: 'Phuc' }, 
   *   'vi'
   ...
   * @description Returns the active language from i18next, falling back to the default if not set
   * @example
   ...
   * @description Changes the language for the entire application instance
   * @example
